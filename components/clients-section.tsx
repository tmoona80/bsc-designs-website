...
export default function ClientsSection() {
  ...
  const scrollToSlide = (direction: "next" | "prev") => {
    ...
    if (direction === "next") {
      const maxSlide = portfolioItems.length - 3
  ...
  const goToSlide = (index: number) => {
    ...
    const slideWidth = scrollContainerRef.current.offsetWidth / 3
  ...
  const totalSlides = Math.max(1, portfolioItems.length - 2)
          ...
          <div className="relative">
            ...
            >
              {portfolioItems.map((item, index) => (
                <div key={index} className="flex-none w-1/3 min-w-0">
